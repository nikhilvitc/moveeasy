  ...
  return (
        <div style={{ minHeight: "100vh", background: "#f1f5f9" }}>
                <div style={{ background: "linear-gradient(135deg, #0f172a, #1e3a8a)", color: "white", padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                          <div><h1 style={{ fontSize: "20px", fontWeight: 800, margin: 0 }}>Seller Dashboard</h1>h1><p style={{ fontSize: "12px", opacity: 0.7, margin: 0 }}>Welcome, {user?.name}</p>p></div>div>
                        <div style={{ display: "flex", gap: "8px" }}>
                                  <button onClick={() => navigate("/map")} style={{ ...st.btn, background: "rgba(255,255,255,0.15)", color: "white" }}>View Map</button>button>
                                  <button onClick={() => navigate("/")} style={{ ...st.btn, background: "rgba(255,255,255,0.15)", color: "white" }}>Home</button>button>
                                  <button onClick={() => { logout(); navigate("/login"); }} style={{ ...st.btn, background: "#ef4444", color: "white" }}>Logout</button>button>
                        </div>div>
                </div>div>
              <div style={{ padding: "20px 24px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                                <h2 style={{ fontSize: "18px", fontWeight: 700, margin: 0 }}>My Listings ({listings.length})</h2>h2>
                                <button onClick={() => setShowAdd(!showAdd)} style={{ ...st.btn, background: "#1e3a8a", color: "white" }}>{showAdd ? "Cancel" : "+ Add Listing"}</button>button>
                      </div>div>
                {showAdd && (
